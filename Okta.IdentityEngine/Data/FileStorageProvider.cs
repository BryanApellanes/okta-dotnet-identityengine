...
namespace Okta.IdentityEngine.Data
{
    public class FileStorageProvider : StorageProvider
    {
        public FileStorageProvider(IIdentityLoggingProvider loggingProvider)
            : base(loggingProvider)
        {
            this.Name = Path.GetFileNameWithoutExtension(Assembly.GetEntryAssembly().Location);
        }

        public FileStorageProvider(string name, IIdentityLoggingProvider loggingProvider)
        ...
        protected override void Save(string key, object value)
        {
            ...
            if (toSave is string json)
            {
                ...
                if(fileToWrite.Directory != null && !fileToWrite.Directory.Exists)
                {
