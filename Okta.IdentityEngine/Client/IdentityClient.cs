...
namespace Okta.IdentityEngine.Client
{
    public class IdentityClient : IIdentityClient
    {
        ...
        public async Task<IIdentitySession> InteractAsync(IIdentitySession? session = null)
        {
            ...
            try
            {
                ...
                {
                    if(identityResponse.Exception != null)
                    {
                        throw identityResponse.Exception;
                    }
                    throw new Exception(identityResponse.ApiErrorDescription);
                }
                ...
                IdentitySession? identitySession = identityResponse.As<IdentitySession>();
                ...
                {
                    throw new InvalidDataException($"Failed to deserialize Okta identity response: \r\n{identityResponse.Raw}");
                }
        ...
        public async Task<IdentityState> IntrospectAsync(IIdentitySession state)
        {
            return await this.IntrospectAsync(state.InteractionHandle);
        }

        public async Task<IdentityState> IntrospectAsync(string interactionHandle)
        {
            try
            {
                ...
                IdentityState model = new IdentityState(responseMessage);
                model.EnsureSuccess();
        ...
        public async Task<TokenResponse> RedeemInteractionCodeAsync(IIdentitySession identitySession, string interactionCode)
        {
