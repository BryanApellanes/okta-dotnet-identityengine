...
namespace Okta.IdentityEngine.Ion
{
    ...
    /// for now Iri extends Uri, in the future this may not be the case.
    /// </summary>
    public class Iri : Uri
    {
        ...
        public static bool IsIri(string url, out Iri? iri, Action<Exception>? exceptionHandler = null)
        {
            ...
            catch (Exception ex)
            {
                ...
                {
                    exceptionHandler = (exception) => Console.WriteLine($"DEBUG: specified value is not a valid Iri ({url}): \r\n{exception.Message}:\r\n\t{exception.StackTrace}");
                }
                exceptionHandler(ex);
                return false;
            }
